...
module Geomora
  module Generators
    class BeamGenerator
      ...
      def generate(beam, storey_elevation, parent_entities)
        Logger.debug("Generating #{beam.id}")

        group = parent_entities.add_group
        group.name = beam.id
        baseline = beam.geometry[:baseline]
        width = beam.geometry[:width].to_f
        height = beam.geometry[:height].to_f
        ...
        normal = Geometry::Vectors.scale(basis[:normal], half_w)
        neg_normal = Geometry::Vectors.scale(basis[:normal], -half_w)
        ...
        write_metadata(group, beam)
        @tags.apply(group, 'Geomora_Beams')
        group
      end
    end
  end
